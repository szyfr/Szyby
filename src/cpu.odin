...
run_cpu :: proc() -> u8 {
	...
	switch opcode {
		...
		case 0x50: // RET
			...
			if program.debug do fmt.printf(" \t- RET\t\t\t| RET $%4X\n", full)
			cycle = 4
		case 0x51: // CALL i16
			...
			if program.debug do fmt.printf(" %2X %2X\t- CALL i16\t\t| CALL $%4X\n", lower, upper, program.regPC+1)
			cycle = 4
			inc_pc()
		//? LD
		case 0x10: // LDZ
		...
		case 0x11: // LD a,x
			...
			if program.debug do fmt.printf("\t\t- LD A,X\t\t| LD A,$%2X\n", program.regX)
			cycle = 2
			inc_pc()
		case 0x12: // LD a,y
			program.regA = program.regY

			if program.debug do fmt.printf("\t\t- LD A,Y\t\t| LD A,$%2X\n", program.regY)
			cycle = 2
			inc_pc()
		case 0x20: // LD a,i8
			program.regA = grab_immediate_u8()

			if program.debug do fmt.printf(" %2X\t- LD A,i8\t\t| LD A,$%2X\n", program.memory[program.regPC], program.memory[program.regPC])
			cycle = 3
			inc_pc()
		case 0x21: // LD x,a
			program.regX = program.regA

			if program.debug do fmt.printf("\t\t- LD X,A\t\t| LD X,$%2X\n", program.regA)
			cycle = 2
			inc_pc()
		case 0x22: // LD y,a
			program.regY = program.regA

			if program.debug do fmt.printf("\t\t- LD Y,A\t\t| LD Y,$%2X\n", program.regA)
			cycle = 2
			inc_pc()
		case 0x30: // LD a,m8
			...
			if program.debug do fmt.printf(" %2X %2X\t- LD A,m8\t\t| LD A,[$%4X]\n", lower, upper, full)
			cycle = 4
			inc_pc()
		case 0x31: // LD m8,a
			full, lower, upper   := grab_immediate_u16()
			...
			if program.debug do fmt.printf(" %2X %2X\t- LD m8,A\t\t| LD [$%4X],$%2X\n", lower, upper, full, program.regA)
			cycle = 4
			inc_pc()
		case 0x32: // LD lp,i16
			full, lower, upper := grab_immediate_u16()
			...
			if program.debug do fmt.printf(" %2X %2X\t- LD LP,i16\t\t| LD LP,$%4X\n", lower, upper, program.regLP)
			cycle = 4
			inc_pc()
		case 0x40: // LD a,[lp]
			program.regA = program.memory[program.regLP]

			if program.debug do fmt.printf(" \t- LD A,[LP]\t\t| LD A,[$%4X]\n", program.regLP)
			cycle = 4
			inc_pc()
		case 0x41: // LD [lp],a
			program.memory[program.regLP] = program.regA

			if program.debug do fmt.printf(" \t- LD [LP],A\t\t| LD [$%4X],$%2X\n", program.regLP, program.regA)
			cycle = 4
			inc_pc()
		case 0x02: // LD lp,sp
			program.regLP = program.regSP
			
			if program.debug do fmt.printf(" \t- LD LP,SP\t\t| LD $%4X,$%4X\n", program.regLP, program.regSP)
			cycle = 4
			inc_pc()
		case 0x42: // LD lp,xy
			program.regLP = (u16(program.regX) << 8) | u16(program.regY)
			
			if program.debug do fmt.printf(" \t- LD LP,XY\t\t| LD LP,$%2X%2X\n", program.regX, program.regY)
			cycle = 4
			inc_pc()
		//? INC
		case 0x05: // INC x
			...
			if program.debug do fmt.printf(" \t- INC X\t\t\t| INC $%2X\n", program.regX-1)
			cycle = 1
			inc_pc()
		case 0x06: // INC y
			...
			if program.debug do fmt.printf(" \t- INC Y\t\t\t| INC $%2X\n", program.regY-1)
			cycle = 1
			inc_pc()
		case 0x07: // INC xy
			...
			if program.debug do fmt.printf(" \t- INC XY\t\t| INC $%4X\n", comb)
			cycle = 2
			inc_pc()
		case 0x15: // INC a
			...
			if program.debug do fmt.printf(" \t- INC A\t\t\t| INC $%2X\n", program.regA-1)
			cycle = 1
			inc_pc()
		case 0x16: // INC lp
			...
			if program.debug do fmt.printf(" \t- INC LP\t\t| INC $%4X\n", program.regLP-1)
			cycle = 1
			inc_pc()
		//? DEC
		case 0x25: // DEC x
			...
			if program.debug do fmt.printf(" \t- DEC X\t\t\t| DEC $%2X\n", program.regX+1)
			cycle = 2
			inc_pc()
		case 0x26: // DEC y
			...
			if program.debug do fmt.printf(" \t- DEC Y\t\t\t| DEC $%2X\n", program.regY+1)
			cycle = 2
			inc_pc()
		case 0x27: // DEC xy
			comb := (u16(program.regX) << 8) | u16(program.regY)
			...
			if program.debug do fmt.printf(" \t- DEC XY\t\t| DEC $%4X\n", comb)
			cycle = 3
			inc_pc()
		case 0x35: // DEC a
			...
			if program.debug do fmt.printf(" \t- DEC A\t\t\t| DEC $%2X\n", program.regA+1+1)
			cycle = 2
			inc_pc()
		case 0x36: // DEC lp
			...
			if program.debug do fmt.printf(" \t- DEC LP\t\t| DEC $%4X\n", program.regLP+1)
			cycle = 2
			inc_pc()
		//? JMP
		case 0x60: // JMP +i8
			...
			if program.debug do fmt.printf(" %2X\t- JMP +$%X\t\t| JMP $%4X\n", var, var, program.regPC+1)
			cycle = 4
			inc_pc()
		case 0x70: // JMP -i8
			var := grab_immediate_u8()
		...
		case 0x61: // JMP  z,+i8
			...
			if program.debug do fmt.printf(" %2X\t- JMP Z,+$%X\t\t| JMP Z,$%4X\n", var, var, program.regPC+1)
			cycle = 4
			inc_pc()
		case 0x71: // JMP  z,-i8
			var := grab_immediate_u8()
		...
		case 0x62: // JMP nz,+i8
			...
			if program.debug do fmt.printf(" %2X\t- JMP NZ,+$%X\t\t| JMP NZ,$%4X\n", var, var, program.regPC+1)
			cycle = 4
			inc_pc()
		case 0x72: // JMP nz,-i8
			var := grab_immediate_u8()
		...
		case 0x63: // JMP  c,+i8
			...
			if program.debug do fmt.printf(" %2X\t- JMP C,+$%X\t\t| JMP C,$%4X\n", var, var, program.regPC+1)
			cycle = 4
			inc_pc()
		case 0x73: // JMP  c,-i8
			var := grab_immediate_u8()
		...
		case 0x64: // JMP nc,+i8
			...
			if program.debug do fmt.printf(" %2X\t- JMP NC,+$%X\t\t| JMP NC,$%4X\n", var, var, program.regPC+1)
			cycle = 4
			inc_pc()
		case 0x74: // JMP nc,-i8
			var := grab_immediate_u8()
		...
		case 0x03: // CMP a,i8
			...
			if program.debug do fmt.printf(" %2X\t- CMP A,i8\t\t| CMP $%2X,$%2X\n", var, program.regA, var)
			cycle = 2
			inc_pc()
		case 0x04: // CMP a,m8
			full, lower, upper := grab_immediate_u16()
			...
			if program.debug do fmt.printf(" %2X %2X\t- CMP A,m8\t\t| CMP $%2X,[$%4X]\n", lower, upper, program.regA, full)
			cycle = 4
			inc_pc()
		case 0x13: // CMP a,x
			...
			if program.debug do fmt.printf(" \t- CMP A,X\t\t| CMP $%2X,$%2X\n", program.regA, program.regX)
			cycle = 4
			inc_pc()
		case 0x14: // CMP a,y
			...
			if program.debug do fmt.printf(" \t- CMP A,Y\t\t| CMP $%2X,$%2X\n", program.regA, program.regY)
			cycle = 4
			inc_pc()
		case 0x23: // CMP lp,i16
			...
			if program.debug do fmt.printf(" %2X %2X\t- CMP LP,i8\t\t| CMP $%4X,$%4X\n", lower, upper, program.regLP, full)
			cycle = 6
			inc_pc()
		case 0x24: // CMP xy,i16
			...
			if program.debug do fmt.printf(" %2X %2X\t- CMP XY,i8\t\t| CMP $%2X%2X,$%2X%2X\n", lower, upper, program.regX, program.regY, upper, lower)
			cycle = 6
			inc_pc()
		//? PUSH
		case 0x08: // PUSH af
			...
			if program.debug do fmt.printf(" \t- PUSH AF\t\t| PUSH $%2X%2X\n", program.regA, program.regF)
			cycle = 2
			inc_pc()
		case 0x18: // PUSH xy
			push(program.regY, program.regX)

			if program.debug do fmt.printf(" \t- PUSH XY\t\t| PUSH $%2X%2X\n", program.regX, program.regY)
			cycle = 2
			inc_pc()
		case 0x28: // PUSH lp
			push(program.regLP)

			if program.debug do fmt.printf(" \t- PUSH LP\t\t| PUSH $%4X\n", program.regLP)
			cycle = 2
			inc_pc()
		//? POP
		case 0x09: // POP af
			...
			if program.debug do fmt.printf(" \t- POP AF\t\t| POP  $%2X%2X\n", program.regA, program.regF)
			cycle = 2
			inc_pc()
		case 0x19: // POP xy
			full, lower, upper := pop()
			...
			if program.debug do fmt.printf(" \t- POP XY\t\t| POP  $%2X%2X\n", program.regX, program.regY)
			cycle = 2
			inc_pc()
		case 0x29: // POP lp
			full, lower, upper := pop()
			...
			if program.debug do fmt.printf(" \t- POP LP\t\t| POP  $%4X\n", program.regLP)
			cycle = 2
			inc_pc()
	}
